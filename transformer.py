...
from transformer_module import Transformer as TransformerPart
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.transformer_part = TransformerPart(
            d_model=d_model,
            nhead=n_head,
    ...
    def src_embedding_and_pos_encoding(self, src_seq):
        return torch.transpose(
            self.src_emb_dropout(self.src_position_enc(self.src_word_emb(src_seq))),
            1,
            0,
        )
    ...
    def trg_embedding_and_pos_encoding(self, trg_seq):
        return torch.transpose(
            self.trg_emb_dropout(self.trg_position_enc(self.trg_word_emb(trg_seq))),
            1,
            0,
        )
    ...
    def forward(
        ...
        """
        src_key_padding_mask = generate_key_padding_mask(src_seq)
        memory_padding_mask = src_key_padding_mask
        trg_key_padding_mask = generate_key_padding_mask(trg_seq)

        src_enc_output = self.src_embedding_and_pos_encoding(
            src_seq
        )  # src_enc_output: `(src_seq_len, batch_size, d_model)`
        trg_enc_output = self.trg_embedding_and_pos_encoding(
            trg_seq
        )  # trg_enc_output: `(trg_seq_len, batch_size, d_model)`
