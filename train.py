...
def prepare_dataloaders(data, opt):
    ...
    # opt.src_pad_idx = Constants.PAD
    # opt.trg_pad_idx = Constants.PAD
...
def train_epoch(model, training_data, optimizer, device, smoothing=True):
    ...
    for batch in tqdm(training_data, mininterval=2, desc=desc, leave=False):
        ...
        # forward
        optimizer.zero_grad()
        pred = model(src_seq, trg_seq, trg_mask)
    ...
    for epoch_i in range(opt.epoch):
        ...
        if opt.save_model:
            if opt.save_mode == 'all':
                model_name = opt.save_model + '_epoch_' + str(epoch_i) + '_accu_{accu:3.3f}.chkpt'.format(accu=100*valid_accu)
...
def main():
    ...
    training_data, validation_data, src_vocab_size, trg_vocab_size = prepare_dataloaders(data, opt)
    opt.src_vocab_size = src_vocab_size
    opt.trg_vocab_size = trg_vocab_size

    transformer = Transformer(src_vocab_size, trg_vocab_size, opt.d_model, opt.d_inner_hid, 
                              opt.d_k, opt.d_v, opt.n_head, opt.n_layers, opt.dropout).to(device)

    optimizer = ScheduledOptim(Adam(transformer.parameters(), betas=(0.9, 0.98), eps=1e-09), 1.0, opt.d_model, opt.n_warmup_steps)
